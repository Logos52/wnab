...
if (isPlaywrightTest) {
  ...
  // wnab divergence: keep the budget data under the backed-up finance tree
  // (~/Documents/Finances/wnab/) instead of upstream's documents/userData dirs.
...
app.on('before-quit', () => {
  // TODO(wnab-bridge): fire an on-close snapshot export here. Doing it safely
  // requires renderer-lifecycle coordination — main would have to
  // event.preventDefault() the quit, ask the renderer to build the snapshot
  // (async `send()` round-trips to the engine background process), wait for it
  // to call save-wnab-snapshot, then re-quit on an ack/timeout. That is
  // fragile during teardown (the engine process below is killed on quit, and a
  // hung/failed build would block exit), so v1 ships the manual Settings button
  // only. See docs/SNAPSHOT-SCHEMA.md §Staleness and the Bridge plan task 5.
  if (serverProcess) {
    serverProcess.kill();
    serverProcess = null;
  }
...
// wnab snapshot export (Obsidian↔wnab bridge). The renderer hands us only the
// JSON string; the main process owns the destination path so the renderer can
// never write to an arbitrary location. The path is the same wnabRoot the
// data-dir patch above computes: ~/Documents/Finances/wnab/snapshot.json — a
// sibling of the budget data, matching docs/SNAPSHOT-SCHEMA.md §Path.
export type SaveWnabSnapshotPayload = {
  contents: string;
};

function getWnabSnapshotPath(): string {
  // Prefer deriving from ACTUAL_DATA_DIR (.../wnab/data) so we stay in lockstep
  // with whatever the data-dir patch set; fall back to recomputing wnabRoot.
  const dataDir = process.env.ACTUAL_DATA_DIR;
  const wnabRoot = dataDir
    ? path.dirname(dataDir)
    : path.join(app.getPath('home'), 'Documents', 'Finances', 'wnab');
  return path.join(wnabRoot, 'snapshot.json');
}

ipcMain.handle(
  'save-wnab-snapshot',
  async (_event, { contents }: SaveWnabSnapshotPayload) => {
    const snapshotPath = getWnabSnapshotPath();
    await mkdir(path.dirname(snapshotPath), { recursive: true });
    await writeFile(snapshotPath, contents, 'utf-8');
  },
);
