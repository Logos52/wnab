...
global.Actual = {
  ...
  saveWnabSnapshot: async contents => {
    // No fixed local path in the browser; fall back to a download so the
    // bridge contract still resolves. The Electron build writes to
    // ~/Documents/Finances/wnab/snapshot.json instead.
    const temp = document.createElement('a');
    temp.style = 'display: none';
    temp.download = 'snapshot.json';
    temp.rel = 'noopener';
