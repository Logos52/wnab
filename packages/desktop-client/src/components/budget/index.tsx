...
import { getHasSeenOnboarding } from '#coaching/firstRun';
...
export function Budget() {
  ...
  // wnab: show the coaching tour once per budget on first run. The "seen" flag
  // lives in localStorage (view-layer only) and is set when the modal closes.
  useEffect(() => {
    if (!initialized || !budgetId) {
      return;
    }
    if (!getHasSeenOnboarding(budgetId)) {
      dispatch(pushModal({ modal: { name: 'onboarding' } }));
    }
  }, [initialized, budgetId, dispatch]);
