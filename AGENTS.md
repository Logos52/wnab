...
## Session handoff — Claude Code ↔ Cowork

This repo is worked by both Claude Code (in-repo execution) and Cowork (planning, review,
cross-workspace coherence). Both read this file via CLAUDE.md. To keep handoffs seamless:

- **End every session clean.** Commit and push before stopping. Never leave a dirty working
  tree or a stale `.git/index.lock`. The other tool reads repo state via git — a clean, pushed
  tree is the handoff.
- **Decisions go to the vault.** Anything non-obvious (the _why_, trade-offs, ruled-out paths)
  is logged to `llm-knowledge-base/journal/`, not left to die in a session transcript.
- **One source of truth per concern.** Code lives here; decisions and PRDs live in the vault.
- **Division of labor.** Claude Code = builds, multi-file edits, tests, in-repo execution.
  Cowork = discovery, PRDs, reviewing CC output, capturing decisions, keeping workspaces coherent.
